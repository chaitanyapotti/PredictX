...
contract PredictXTestCommon is CommonOptimisticOracleV3Test {
    PredictX public predictionMarket;
    string outcome1 = "Red";
    ...
    function _commonPredictionMarketSetUp() public {
        _commonSetup();
        predictionMarket = new PredictX(address(finder), address(defaultCurrency), address(optimisticOracleV3));
    ...
    function _mintAndSwapOutcomeTokens(bytes32 marketId) internal {
        PredictX.Market memory market = predictionMarket.getMarket(marketId);
